...
> **Importante:** A continuación se presenta un resumen que contiene códigos y fragmentos que fueron replicados del capítulo 9 del libro "Applied Spatial Data Analysis with R" de los autores Roger S. Bivand, Edzer Pebesma y Virgilio Gómez-Rubio. Esto se hace con el propósito de aprender a analizar datos en área y de demostrar la lectura del texto para el curso de estadística espacial.
...
library(spatialreg)
library(devtools)
# Instalando el paquete McSpatial desde GitHub
install_github("cran/McSpatial")
library(McSpatial)
library(coda)
...
En álgebra lineal una matriz dispersa o matriz rala o matriz hueca es una matriz de gran tamaño en la que la mayor parte de sus elementos son cero.

<br>

Hay varios paquetes que brindan soporte para matrices dispersas, entre los cuales estan:


- [Matrix](https://cran.r-project.org/web/packages/Matrix/index.html) (paquete recomendado) 

- [as_dgRMatrix_listw](https://www.rdocumentation.org/packages/spdep/versions/1.0-2/topics/as_dgRMatrix_listw) (interfaz entre los objetos de la clase Matrix y las listas de pesos): convierte un objeto listw en una matriz dispersa a partir de una matriz ordenada, comprimida y orientada a filas, como un objeto **dgRMatrix**(una subclase de la clase virtual **RsparseMatrix**).
...
* Una función que se utiliza mucho en las funciones de prueba y ajuste de modelos es [listw2U](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/moran.test) (Prueba I de Moran para la autocorrelación espacial) que devuelve un objeto listw simétrico que representa la matriz de ponderaciones espaciales $\frac{1}{2}(W + W^T)$.



**Nota:** Los objetos vecinos y de pesos producidos en otro software se pueden importar a R sin dificultad y dichos objetos también se pueden exportar a otro software.


**Ejemplo.**

Se generan algunos archivos en GeoDa a partir de las secciones del censo de Syracuse (región de estudio) escritas como un archivo de forma, con el centroide utilizado aquí almacenado en el marco de datos. Los dos primeros son para vecinos de contigüidad, utilizando los criterios de dama y torre, respectivamente. Estos archivos de formato GAL contienen solo información de vecinos y se describen en detalle en el archivo de ayuda que acompaña a la función [read.gal](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/read.gal) (lee un archivo GAL lattice en una lista de vecinos) .

```{r}
# Lectura del archivo en formato GAL generado en GeoDa
Sy14_nb <- read.gal("Sy_GeoDa1.GAL")

# Validación de la igualdad de los formatos
# Sy0_nb: subconjunto de la ciudad de Syracuse
isTRUE(all.equal(Sy0_nb, Sy14_nb, check.attributes = FALSE))
```
<br>

La función [write.nb.gal](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/write.nb.gal) se utiliza para escribir archivos en formato GAL a partir de objetos nb. 


(Nota: un objeto de clase "nb" es una lista de vectores enteros que contienen identificadores de números de región vecina. GAL Es un formato de datos biológicos en la forma de una lista de matriz.)



[GeoDa](https://geodacenter.github.io/) también crea archivos en formato **GWT**, descritos en la documentación de GeoDa, ésta también contienen información de distancia para el vínculo entre las áreas y se almacenan en una representación dispersa de tres columnas. Se pueden leer usando [read.gwt2nb](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/read.gwt2nb) (leer y escribir archivos vecinos espaciales), aquí para un esquema de cuatro vecinos más cercanos, y solo usando los enlaces vecinos.
...
Un conjunto similar de funciones está disponible para intercambiar pesos espaciales con la libreria [the Spatial Econometrics](http://www.econ.uiuc.edu/~lab/workshop/Spatial_in_R.html). La representación dispersa de los pesos es similar al formato **GWT** y se puede importar usando [read.dat2listw](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/read.gwt2nb) del paquete **spdep**.


* La exportación a tres formatos diferentes pasa por la función [listw2sn](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/listw2sn), que convierte un objeto de pesos espaciales en una representación dispersa de tres columnas. 

* El marco de datos de salida se puede escribir como un archivo de formato **GWT** con [write.sn2gwt](https://www.rdocumentation.org/packages/spdep/versions/0.1-13/topics/read.gwt2nb) o como una representación de texto de una matriz dispersa para Matlab™ con [write.sn2dat](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/read.gwt2nb).


* Los archivos escritos usando [write.sn2gwt](https://www.rdocumentation.org/packages/spdep/versions/0.1-13/topics/read.gwt2nb) se pueden leer en Stata™ con spmat import W using.

* Hay una función llamada [listw2WB](https://www.rdocumentation.org/packages/spdep/versions/1.1-8/topics/nb2WB) para crear una lista de pesos espaciales para **WinBUGS**, que se escribirán en un archivo usando [dput](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/dput). 

* Para objetos vecinos, se puede usar [nb2WB](https://www.rdocumentation.org/packages/spdep/versions/1.1-8/topics/nb2WB), configurando todos los pesos a la unidad. De manera similar, un objeto vecino puede exportarse a un archivo con [nb2INLA](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/nb2INLA), para pasar datos al argumento del gráfico para el modelo "bym" en ajuste usando inla. 


* También es posible usar [nb2gra](https://rdrr.io/github/datacamp/R2BayesX/man/nbAndGraConversion.html) en el paquete [BayesX](https://cran.r-project.org/web/packages/BayesX/index.html) para convertir objetos **nb** al formato gráfico **gra**.


* El [mat2listw](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/mat2listw) se puede usar para revertir el proceso, cuando una matriz de pesos se ha leído en R y se debe convertir en un objeto vecino y de lista de pesos. Desafortunadamente, esta función no establece el estilo del objeto listw en un valor conocido, usando **M** para señalar esta falta de conocimiento. Entonces es habitual reconstruir el objeto listw, tratando el componente de vecinos como un objeto **nb**, el componente de pesos como una lista de pesos generales y estableciendo el estilo en la función [nb2listw](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/nb2listw) directamente. 


* Finalmente, hay una función [nb2lines](https://www.rdocumentation.org/packages/spdep/versions/1.2-4/topics/nb2lines) para convertir listas de vecinos en objetos **SpatialLinesDataFrame**, dadas las coordenadas de los puntos que representan las áreas. Esto permite trazar objetos vecinos de forma alternativa y, si es necesario, exportarlos como archivos de forma.
...
En la figura anterior (Fig. 9.3), se hizo uso de [listw2mat](https://rdrr.io/cran/spdep/man/nb2mat.html) para convertir un objeto de pesos espaciales en una matriz densa para su visualización. La misma función se usa para construir una representación densa de la matriz $(I − ρW)$ para simular la autocorrelación espacial dentro de la función [invIrW](https://www.rdocumentation.org/packages/spdep/versions/0.7-9/topics/invIrM), donde $W$ es una matriz de pesos, $ρ$ es un coeficiente de autocorrelación espacial e $I$ es la matriz identidad. Este enfoque no impone condiciones estrictas sobre la matriz a invertir (solo que no sea singular), y solo se aplica a simulaciones de un proceso autorregresivo simultáneo. 
...
Ya que aprendimos a construir pesos espaciales, podemos comenzar a usarlos para probar la presencia de autocorrelación espacial. A continuación revisaremos los supuestos que se hacen en este test, se estará usando el I de Moran como ejemplo, sin embargo, las consecuencias también aplican para otras pruebas.

- Las pruebas asumen que el modelo de media de los datos elimina patrones espaciales sistemáticos de los datos.

Si estamos examinando datos ecológicos, pero ignoramos los factores ambientales como la temperatura, precipitación o elevación, no deberíamos sorprendernos si los datos parecen mostrar autocorrelación espacial (para una discusión, ver Bivand, 2008, pp. 9–15).

Tal especificación errónea del modelo de media no es poco frecuente y puede ser inevitable cuando no se dispone de las observaciones sobre las variables necesarias para especificarlo correctamente.

- Otro problema que puede surgir es que los pesos espaciales que usamos para las pruebas, no son los que generaron la autocorrelación - los pesos elegidos pueden, por ejemplo, no adaptarse a las escalas reales de interacción entre entidades de área.

Esto es un reflejo de la especificación incorrecta del modelo de la varianza de los residuales para el modelo de media, que también puede incluir hacer suposiciones de distribución que no son apropiadas para los datos. Algunos de estos casos pueden abordarse mediante la transformación de los datos y el uso de la estimación ponderada, pero en cualquier caso, se necesita cuidado al interpretar la autocorrelación espacial aparente que en realidad puede derivarse de una especificación incorrecta.

---

Empezaremos con un ejemplo con la variable simulada para las secciones censales de Syracuse. Dado que se sabe que la variable de entrada se extrae al azar de la distribución Normal podemos manipularla para ver qué sucede con los resultados de las pruebas en diferentes condiciones. 

La prueba que se usará en esta discusión es la I de Moran:

![](I_moran.png)

donde $y_i$ es la i-ésima observación, $\bar{y}$ es la media de la variable de interés y $w_{ij}$ es el peso espacial del enlace entre i y j. Centrarse en la media es equivalente a afirmar que el modelo correcto tiene una media constante y que cualquier patrón restante después del centrado es causado por las relaciones espaciales codificadas en los pesos espaciales.


A continuación, se calcula el I de Moran para 5 configuraciones diferentes, cada función nos devuelve:

El valor estimado para I
Su valor esperado, (el cual es -1/(n-1) para los casos centrados en la media)
La varianza de la estadística bajo aleatorización
La desviación estándar
Y el p-valor de la prueba para la alternativa que I > E(I)
...
no hay rastro de dependencia espacial con estos pesos.

* Caso con variable espacialmente correlacionada

```{r}
moran_a <- moran.test(autocorr_x, listw = Sy0_lw_W)
```

Como era de esperarse se muestra un resultado significativo para estos pesos espaciales.

* Caso usando pesos espaciales que difieren de los usados para generar la autocorrelación espacial.
...
Podemos ver que el valor de I cae, y aunque es marginalmente significativo, vale la pena recordar que, si el proceso de generación hubiera sido menos fuerte, podríamos haber llegado a una conclusión equivocada basada en la elección de pesos espaciales, no coincidiendo con el proceso de generación real.

* Las 2 siguientes muestran lo que puede suceder cuando nuestra suposición de una media constante es errónea 

Al introducir una tendencia suave que sube de oeste a este en la variable aleatoria no correlacionada, tenemos una situación en la que no hay una autocorrelación espacial subyacente, solo una tendencia lineal simple. Si asumimos una media constante, llegamos a la conclusión incorrecta.
...
Esta ultima muestra cómo volvemos a los residuos no correlacionados al incluir la tendencia en la media, y nuevamente tenemos residuos no correlacionados.
...
Lo anterior muestra cuán importante puede ser comprender que las pruebas de autocorrelación espacial también pueden reaccionar a un modelo mal especificado de la media, y que la omisión de una variable con patrón espacial de la función media "parecerá" una autocorrelación espacial para las pruebas.

<br>

### 9.3.1 Pruebas globales

La I de Moran, `moran.test`, es quizás la prueba global más común y, por esta razón, la seguimos usando aquí. Otras pruebas globales implementadas en el paquete spdep incluyen:

- Geary's C (geary.test()), 
- Getis-Ord G global (globalG.test()) 
- Y la prueba espacial general de productos cruzados Mantel, que incluye Moran's I, Geary's C, y la variante Sokal de la C de Geary como formas alternativas (sp.mantel.mc()). 

Todos estos son para variables continuas.

Aquí se evaluan las siguientes hipótesis:

H0: No hay dependencia espacial para los pesos espaciales elegidos.

Ha: Hay dependencia espacial para los pesos espaciales elegidos.

Podría parecer que Monte Carlo o pruebas equivalentes basadas en permutaciones bootstrap, en las que los valores de la variable de interés se asignan aleatoriamente a entidades espaciales, brindarían protección contra errores de inferencia.

En realidad, debido a que las pruebas de autocorrelación espacial son sensibles al patrón espacial en la variable de interés de cualquier fuente, no son necesariamente, como vimos anteriormente, buenas guías para decidir qué está sucediendo en el proceso de generación de datos.

Otro problema para el que no existe un mejor consejo actual es cómo proceder si algunas entidades del área no tienen vecinos. De forma predeterminada, las funciones de prueba en spdep no aceptan ponderaciones espaciales con entidades no vecinas a menos que el argumento zero.policy se establezca en TRUE.

A continuación, utilizado el conjunto de datos de los ocho condados del censo en el estado de Nueva York se realizan algubnos ejemplos: 

El primer ejemplo es probar el número de casos por tramo censal para la autocorrelación usando el estilo predeterminado de ponderaciones espaciales de estandarización de filas y usando la suposición de aleatorización analítica al calcular la varianza de la estadística. 

```{r}
moran.test(NY8$Cases, listw = nb2listw(NY_nb))
```

El resultado, como vemos, es que el patrón espacial de la variable de interés es significativo, y es muy probable que los tramos vecinos tengan valores similares por cualquier motivo.


Cambiando el estilo de los pesos espaciales para que todos los pesos sean iguales y sumando el número de observaciones. 
...
vemos que el valor de probabilidad resultante se reduce unas 20 veces.

Por defecto, moran.test utiliza la suposición de aleatorización, que difiere de la suposición de normalidad más simple al introducir un término de corrección basado en la curtosis de la variable de interés.

```{r}
moran.test(NY8$Cases, listw = lw_B, randomisation = FALSE)
```

En este caso, hay poca diferencia y los dos arrojan resultados similares.

Siguiente se muestra que la prueba estándar bajo normalidad es, de hecho, la misma prueba que la prueba de Moran para los residuos de regresión del modelo, incluyendo solo la intersección.
...
* Usando la aproximación de Saddlepoint

En general, los métodos exactos y de Saddlepoint hacen poca diferencia en los resultados de las pruebas globales cuando el número de entidades espaciales no es pequeño, como aquí.

```{r}
lm.morantest.sad(lm(Cases ~ 1, NY8), listw = lw_B)
```

* Usando la prueba de Monte Carlo, una prueba de permutación en el que los valores observados se asignan aleatoriamente a los tramos.

```{r}
lm.morantest.exact(lm(Cases ~ 1, NY8), listw = lw_B)
```

* Boostrap parametrico incluyendo una constante de riesgo Poisson.
...
Histogramas para los valores del I de Moran bajo permutaciones aleatorias y muestras parametricas del valores esperado de una constante de riesgo.


Otro enfoque es trazar y tabular valores de una medida de autocorrelación espacial para órdenes superiores de vecinos o bandas de vecinos más distantes donde las entidades espaciales son puntos.
...
El panel derecho de la figura presenta la salida de la función correlog en el paquete pgirmess de Patrick Giraudoux; la función es un contenedor para dnearneigh y moran.test. La función selecciona automáticamente bandas de distancia de casi 10 km, que abarcan toda el área de estudio. En este caso, las dos primeras bandas de 0 a 10 y de 10 a 20 km tienen valores significativos.
...
Las pruebas globales de autocorrelación espacial se calculan a partir de las relaciones locales entre los valores observados en una entidad espacial y sus vecinos, para la definición de vecino elegida. Debido a esto, podemos dividir las medidas globales en sus componentes y, por extensión, construir pruebas localizadas destinadas a detectar "grupos" (observaciones con vecinos muy similares) y "puntos calientes" (observaciones con vecinos muy diferentes).

Primero, examinemos un diagrama de dispersión de Moran de la variable de recuento de casos de leucemia; para una discusión de diagramas de dispersión de Moran.

* Diagrama de dispersión de Moran de la incidencia de leucemia
...
Dado que el I de Moran global es, al igual que los coeficientes de correlación similares, una relación lineal, también podemos aplicar técnicas estándar para detectar observaciones con una influencia inusualmente fuerte en la pendiente.

Esto significa que podemos ver si las relaciones locales particulares pueden influir en la pendiente más que proporcionalmente.

![](mapa1_local.png)

El mapa anterior, muestra las zonas con influencia significativa (usando criterios estándar) codificadas por su cuadrante en el diagrama de dispersión de Moran.


* I de Moran local:

Los valores de la I de Moran local se construyen como los n componentes sumados para llegar a la I de Moran global:

![](I_moran_local.png)

Al igual que con la estadística global, las estadísticas locales pueden probarse en busca de divergencia de los valores esperados, bajo suposiciones de normalidad y aleatorización analíticamente, y utilizando aproximaciones de Saddlepoint y métodos exactos. Los dos últimos métodos pueden ser importantes porque el número de vecinos de cada observación es muy pequeño y esto, a su vez, puede dificultar la adopción del supuesto de normalidad.

* Valores locales de $I_i$ de Moran calculados directamente.
...
* Valores locales de $I_i$ de Moran usando la hipótesis de riesgo constante
...
La Figura a continuación muestra los dos conjuntos de valores del $I_i$ local de Moran, calculados de la manera estándar y utilizando el supuesto de Poisson para la hipótesis de riesgo constante.

![](mapa2_local.png)

Se puede ver que hay algunos cambios de signo entre los mapas, con los valores de la hipótesis de riesgo constante algo más alejados de cero.


También podemos construir una prueba simple de Monte Carlo de los valores locales de $I_i$ de Moran de la hipótesis de riesgo constante, simulando mucho como en el caso global, pero ahora conservando todos los resultados locales.
...
* Valores de probabilidad para todas las secciones censales

Finalmente, se examinan los valores de probabilidad locales de $I_i$ de Moran para varios métodos de cálculo. Moran local: supuestos de normalidad y aleatorización, aproximación de Saddlepoint, valores exactos e hipótesis de riesgo constante.

![](mapa3_local.png)

- Parece que el uso del enfoque de riesgo constante maneja la heterogeneidad en los conteos mejor que las otras alternativas.

- Encontrar la autocorrelación espacial no es un objetivo en sí mismo, ya sea local o global, sino solo un paso en un proceso que conduce a un modelo adecuado.

<br>


## 9.4 Ajuste de modelos de datos en área 
...
```{r}
set.seed(987654)
imps <- spatialreg::impacts(obj = nymix, tr = trMat, R = 1999)
imps
...
En general observamos que el análisis de datos en áreas tiene mucha utilidad en la practica y que además es muy utilizado en ciencias medicas, econométria  y en muchas ramas científicas, nos queda también que incluir covariables en los modelos que estiman la autocorrelación es de mucha utilidad para dar conclusiones acertadas, por otro lado, se recuerda la importancia de especificar los pesos y los vecinos, ya que si se hace de manera incorrecta se podría llegar a conclusiones erróneas. 
